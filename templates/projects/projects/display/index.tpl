...
<tr>
	<td valign="top" style="padding-right:5px;">
		<fieldset>
			<legend>{'projects.common.project'|devblocks_translate}</legend>
			...
			*}
			...
			<!-- Toolbar -->
			<button type="button" id="btnProjectEdit"><span class="cerb-sprite sprite-document_edit"></span> Edit</button>
			{*
...
<div id="projectTabs">
	<ul>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextComments&context=cerberusweb.contexts.projects.project&id={$project->id}{/devblocks_url}">{'common.comments'|devblocks_translate|capitalize}</a></li>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextLinks&context=cerberusweb.contexts.projects.project&id={$project->id}{/devblocks_url}">{'common.links'|devblocks_translate}</a></li>

		{$tabs = [comments, links]}
		
		{foreach from=$tab_manifests item=tab_manifest}
			{$tabs[] = $tab_manifest->params.uri}
			<li><a href="{devblocks_url}ajax.php?c=projects&a=showProjectTab&ext_id={$tab_manifest->id}&project_id={$project->id}{/devblocks_url}"><i>{$tab_manifest->params.title|devblocks_translate}</i></a></li>
		{/foreach}
	</ul>
</div> 
<br>
...
<script type="text/javascript">
	$(function() {
		var tabs = $("#projectTabs").tabs( { selected:{$tab_selected_idx} } );
		
		$('#btnProjectEdit').bind('click', function() {
			$popup = genericAjaxPopup('peek','c=projects&a=showProjectPeek&id={$project->id}',null,false,'550');
			$popup.one('projects_project_save', function(event) {
				event.stopPropagation();
				document.location.href = '{devblocks_url}c=projects&a=project&id={$project->id}{/devblocks_url}';
			});
		})
	});
</script>
