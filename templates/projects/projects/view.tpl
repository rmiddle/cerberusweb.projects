...
<input type="hidden" name="context_id" value="cerberusweb.contexts.projects.project">
	...
	<tbody onmouseover="$(this).find('tr').addClass('hover');" onmouseout="$(this).find('tr').removeClass('hover');">
		<tr class="{$tableRowClass}">
			...
			<td colspan="{math equation="x" x=$smarty.foreach.headers.total}">
				<a href="{devblocks_url}c=projects&a=project&id={$result.p_id}{/devblocks_url}" class="subject">{$result.p_name}</a><a href="javascript:;" onclick="genericAjaxPopup('peek','c=projects&a=showProjectPeek&view_id={$view->id}&id={$result.p_id}', null, false, '500');"><span class="ui-icon ui-icon-newwin" style="display:inline-block;vertical-align:middle;" title="{$translate->_('views.peek')}"></span></a>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
...
<table cellpadding="2" cellspacing="0" border="0" width="100%" id="{$view->id}_actions">
	...
	<tr>
		<td colspan="2">
			...
			<button type="button" onclick="genericAjaxPopup('peek','c=projects&a=showProjectBulkUpdate&view_id={$view->id}&ids=' + Devblocks.getFormEnabledCheckboxValues('viewForm{$view->id}','row_id[]'),null,false,'550');"><span class="cerb-sprite sprite-folder_gear"></span> {'common.bulk_update'|devblocks_translate|lower}</button>
		</td>
	</tr>
	{/if}
	<tr>
