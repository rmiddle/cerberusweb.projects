...
<div id="projectTabs">
	<ul>
		{$tabs = []}
		{$point = Extension_ProjectsTab::POINT}
		
		{foreach from=$tab_manifests item=tab_manifest}
			{if !isset($tab_manifest->params.acl) || $worker->hasPriv($tab_manifest->params.acl)}
				{$tabs[] = $tab_manifest->params.uri}
				<li><a href="{devblocks_url}ajax.php?c=projects&a=showTab&ext_id={$tab_manifest->id}&point={$point}&request={$response_uri|escape:'url'}{/devblocks_url}">{$tab_manifest->params.title|devblocks_translate}</a></li>
			{/if}
		{/foreach}
...
<script type="text/javascript">
	$(function() {
		var tabs = $("#projectTabs").tabs( { selected:{$selected_tab_idx} } );
	});
</script>
