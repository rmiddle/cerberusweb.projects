...
<input type="hidden" name="a" value="doQuestionsQuickSearch">
...
{if 1 || $active_worker->hasPriv('xxx')}<button type="button" onclick="genericAjaxPopup('peek','c=projects&a=showQuestionPeek&id=0&view_id={$view->id}',null,false,'500');"><span class="cerb-sprite sprite-add"></span> {$translate->_('common.add')|capitalize}</button>{/if}
</form>
