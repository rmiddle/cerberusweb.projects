<ul class="submenu">
	<li><a href="{devblocks_url}c=projects&a=ideas{/devblocks_url}">{'projects.common.ideas'|devblocks_translate|lower}</a></li>
</ul>
...
<tr>
	<td valign="top" style="padding-right:5px;">
		<fieldset>
			<legend>{'projects.common.idea'|devblocks_translate}: {$idea->mask}</legend>
			<h1><b>{$idea->summary}</b></h1> 
			...
			<form action="{devblocks_url}{/devblocks_url}" onsubmit="return false;">
				...
				*}
				...
				<!-- Toolbar -->
				<button type="button" id="btnProjectSuggestionEdit"><span class="cerb-sprite sprite-document_edit"></span> Edit</button>
				{*
...
<div id="projectSuggestionTabs">
	<ul>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextComments&context=cerberusweb.contexts.projects.idea&id={$idea->id}{/devblocks_url}">{'common.comments'|devblocks_translate|capitalize}</a></li>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextLinks&context=cerberusweb.contexts.projects.idea&id={$idea->id}{/devblocks_url}">{'common.links'|devblocks_translate}</a></li>

		{$tabs = [comments, links]}
		
		{foreach from=$tab_manifests item=tab_manifest}
			{$tabs[] = $tab_manifest->params.uri}
			<li><a href="{devblocks_url}ajax.php?c=projects&a=showProjectTab&ext_id={$tab_manifest->id}&project_id={$idea->id}{/devblocks_url}"><i>{$tab_manifest->params.title|devblocks_translate}</i></a></li>
		{/foreach}
	</ul>
</div> 
<br>
...
<script type="text/javascript">
	$(function() {
		var tabs = $("#projectSuggestionTabs").tabs( { selected:{$tab_selected_idx} } );
		
		$('#btnProjectSuggestionEdit').bind('click', function() {
			$popup = genericAjaxPopup('peek','c=projects&a=showSuggestionPeek&id={$idea->id}',null,false,'550');
			$popup.one('projects_idea_save', function(event) {
				event.stopPropagation();
				document.location.href = '{devblocks_url}c=projects&a=idea&id={$idea->id}{/devblocks_url}';
			});
		})
	});
</script>
