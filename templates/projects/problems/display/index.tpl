<ul class="submenu">
	<li><a href="{devblocks_url}c=projects&a=problems{/devblocks_url}">{'projects.common.problems'|devblocks_translate|lower}</a></li>
</ul>
...
<tr>
	<td valign="top" style="padding-right:5px;">
		<fieldset>
			<legend>{'projects.common.problem'|devblocks_translate}: {$problem->mask}</legend>
			<h1><b>{$problem->summary}</b></h1>
			...
			*}
			...
			<!-- Toolbar -->
			<button type="button" id="btnProjectProblemEdit"><span class="cerb-sprite sprite-document_edit"></span> Edit</button>
			{*
...
<div id="projectProblemTabs">
	<ul>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextComments&context=cerberusweb.contexts.projects.problem&id={$problem->id}{/devblocks_url}">{'common.comments'|devblocks_translate|capitalize}</a></li>
		<li><a href="{devblocks_url}ajax.php?c=internal&a=showTabContextLinks&context=cerberusweb.contexts.projects.problem&id={$problem->id}{/devblocks_url}">{'common.links'|devblocks_translate}</a></li>

		{$tabs = [comments, links]}
		
		{foreach from=$tab_manifests item=tab_manifest}
			{$tabs[] = $tab_manifest->params.uri}
			<li><a href="{devblocks_url}ajax.php?c=projects&a=showProjectTab&ext_id={$tab_manifest->id}&project_id={$problem->id}{/devblocks_url}"><i>{$tab_manifest->params.title|devblocks_translate}</i></a></li>
		{/foreach}
	</ul>
</div> 
<br>
...
<script type="text/javascript">
	$(function() {
		var tabs = $("#projectProblemTabs").tabs( { selected:{$tab_selected_idx} } );
		
		$('#btnProjectProblemEdit').bind('click', function() {
			$popup = genericAjaxPopup('peek','c=projects&a=showProblemPeek&id={$problem->id}',null,false,'550');
			$popup.one('projects_problem_save', function(event) {
				event.stopPropagation();
				document.location.href = '{devblocks_url}c=projects&a=problem&id={$problem->id}{/devblocks_url}';
			});
		})
	});
</script>
